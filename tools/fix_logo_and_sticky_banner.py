...
IMPROVED_LOGO_HTML = '''
        ...
        <script>
            function handleLogoError(img) {
                console.log('Clearbit logo failed, trying alternatives...');
                
                // 备用方案1: 尝试不带www的域名
                if (!img.dataset.tried1) {
                    img.dataset.tried1 = 'true';
                    const domain = img.src.split('clearbit.com/')[1];
                    const cleanDomain = domain.replace('www.', '');
                    img.src = 'https://logo.clearbit.com/' + cleanDomain;
                    return;
                }
                
                // 备用方案2: 使用Google Favicon API
                ...
                // 备用方案3: 显示银行名称文字
                if (!img.dataset.tried3) {
                    img.dataset.tried3 = 'true';
...
def update_page(file_path):
    ...
    try:
        ...
        # 检查是否已更新
        if 'id="stickyBanner"' in content and 'handleLogoError' in content:
        ...
        old_banner_pattern = r'<!-- Free Trial Banner -->.*?</div>\s*</div>'
        if re.search(old_banner_pattern, content, re.DOTALL):
            content = re.sub(
                old_banner_pattern,
                STICKY_BANNER_HTML.strip(),
                content,
                flags=re.DOTALL,
                count=1
            )
        ...
        # 2. 替换Logo为改进版本
        old_logo_pattern = r'<!-- Bank Logo.*?</div>'
        improved_logo = IMPROVED_LOGO_HTML.format(
            domain=bank_domain,
            bank_name=bank_name
        ).strip()
