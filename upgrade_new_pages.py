...
NEW_PAGES = [
    # 亚洲Batch 1 (6个)
    'mufg-bank-statement-v2.html',
    'kb-kookmin-statement-v2.html',
    'smbc-bank-statement-v2.html',
    'shinhan-bank-statement-v2.html',
    'mizuho-bank-statement-v2.html',
    'hana-bank-statement-v2.html',
    # 欧洲 (6个)
    'deutsche-bank-statement-v2.html',
    'ing-bank-statement-v2.html',
    'commerzbank-statement-v2.html',
    'rabobank-statement-v2.html',
    'abn-amro-statement-v2.html',
    'dz-bank-statement-v2.html',
    # 亚洲Batch 2 (6个)
    'woori-bank-statement-v2.html',
    'bank-of-taiwan-statement-v2.html',
    'ctbc-bank-statement-v2.html',
    'cathay-bank-statement-v2.html',
    'hang-seng-bank-statement-v2.html',
    'boc-hk-statement-v2.html'
]

BANK_NAMES = {
    'mufg': ('MUFG Bank', 'Mitsubishi UFJ Financial Group'),
    'kb-kookmin': ('KB Kookmin Bank', 'KB Kookmin Bank'),
    'smbc': ('SMBC', 'Sumitomo Mitsui Banking Corporation'),
    'shinhan': ('Shinhan Bank', 'Shinhan Bank'),
    'mizuho': ('Mizuho Bank', 'Mizuho Bank'),
    'hana': ('Hana Bank', 'Hana Bank'),
    'deutsche': ('Deutsche Bank', 'Deutsche Bank AG'),
    'ing-bank': ('ING Bank', 'ING Bank N.V.'),
    'commerzbank': ('Commerzbank', 'Commerzbank AG'),
    'rabobank': ('Rabobank', 'Rabobank'),
    'abn-amro': ('ABN AMRO', 'ABN AMRO Bank N.V.'),
    'dz-bank': ('DZ Bank', 'DZ Bank AG'),
    'woori': ('Woori Bank', 'Woori Bank'),
    'taiwan': ('Bank of Taiwan', '臺灣銀行'),
    'ctbc': ('CTBC Bank', '中國信託商業銀行'),
    'cathay': ('Cathay Bank', '國泰世華銀行'),
    'hang-seng': ('Hang Seng Bank', '恒生銀行'),
    'boc-hk': ('BOC Hong Kong', '中國銀行(香港)')
}

def get_bank_name(filename):
    """从文件名提取银行名称"""
    for key, (short_name, full_name) in BANK_NAMES.items():
        if key in filename:
            return short_name, full_name
    return 'Bank', 'Bank'

def generate_features_section(bank_name, full_name):
    """生成Features Section"""
    return f'''
    <!-- Features Section -->
    <section style="padding: var(--space-20) var(--space-6); background: white;">
        <div style="max-width: 1200px; margin: 0 auto;">
            ...
            <p style="text-align: center; color: var(--gray-600); font-size: var(--text-lg); margin-bottom: var(--space-16); max-width: 800px; margin-left: auto; margin-right: auto;">
                Industry-leading AI technology specifically trained on {full_name} statement formats
            </p>
...
def upgrade_page(filename):
    ...
    try:
        ...
        bank_name, full_name = get_bank_name(filename)
        features_html = generate_features_section(bank_name, full_name)
        
        # 在"Related Pages Section"或</body>前插入Features
...
for filename in NEW_PAGES:
    ...
    bank_name, _ = get_bank_name(filename)
    
    if ok:
        print(f"  ✅ {bank_name:25s} - {filename}")
        success += 1
    elif msg == "已升级":
        print(f"  ⏭️  {bank_name:25s} - 已升级，跳过")
        skip += 1
    else:
