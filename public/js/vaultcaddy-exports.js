(() => {
  // src/services/accounting_export.ts
  var AccountingExporter = class {
    /**
...
`;
  ...
  // src/services/audit_excel_generator.ts
  var AuditExcelGenerator = class {
    /**
     * 生成並下載審計專用 Excel
     * @param records 已審核的開支紀錄數組
     * @param companyName 公司名稱 (用於檔案命名)
     */
    generateAndDownload(records, companyName) {
      const wsData = [
        [
          "\u4EA4\u6613\u65E5\u671F (Date)",
          "\u5546\u6236\u540D\u7A31 (Merchant)",
      ...
      records.forEach((record, index) => {
        ...
        const cellRef = utils.encode_cell({ r: rowIndex, c: 7 });
        if (!ws[cellRef]) {
      ...
      utils.book_append_sheet(wb, ws, "Audit_Trail_\u5BE9\u8A08\u8ECC\u8DE1");
      const fileName = `Audit_Report_${companyName}_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.xlsx`;
      writeFileSync(wb, fileName);
    }
  };

  // src/services/ixbrl_generator.ts
  var iXBRLGenerator = class {
    companyBrNumber;
    companyName;
    periodEnd;
    // e.g., "2026-03-31"
...
<body>
  ...
  // src/services/export_bundle_entry.js
  window.VaultCaddyExports = {
    AccountingExporter,
    AuditExcelGenerator,
    iXBRLGenerator
  };
